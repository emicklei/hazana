...
var (
	...
	oSample         = flag.Int(fSample, 0, "test your attack implementation with a number of sample calls (default 0). Your program exits after this")
	oRampupStrategy = flag.String(fRampupStrategy, defaultRampupStrategy, `set the rampup strategy, possible values are {linear, "exp2 [keep=S] [max-factor=F]"}`)
